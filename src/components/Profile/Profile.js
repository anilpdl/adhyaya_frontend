...
class Profile extends Component {
  ...
  fetchUserDetails = () => {
    ...
    UserApi.getDetails(id).then(({ data }) => {
      const { user } = data;
      const { first_name: firstName, last_name: lastName, middle_name: middleName } = user;
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const {
      firstName, middleName, gender, lastName
    } = this.state;
    ...
    if (isFormValid) {
      ...
      UserApi.update(formData, id).then(({ data }) => {
  ...
  render() {
    ...
    const {
      firstName, middleName, lastName, gender, errors, email
    } = this.state;
